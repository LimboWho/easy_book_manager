        ...
        <div class="container">
            ...
            >
                ...
                <el-table-column label="商品价格">
                    <template #default="scope">￥{{ scope.row.sellingprice }}</template>
                </el-table-column>
                <el-table-column label="商品图)" align="center">
                    <template #default="scope">
                        <el-image
                            class="table-td-thumb"
                            :src="scope.row.goodscoverimg1"
                            :preview-src-list="[scope.row.goodscoverimg1]"
                        ></el-image>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="状态" align="center">
                    <template #default="scope">
                        <el-tag
                            :type="
                                scope.row.merchantsid === '1'
                                    ? 'success'
                                    : scope.row.merchantsid === '2'
                                    ? 'danger'
                                    : ''
                            "
                        >{{ scope.row.merchantsid }}</el-tag>
                    </template>
                </el-table-column>
...
import getGoodBySear from '@/api/search'
import {modifyGood,getGoods} from '@/api/goods'
export default {
    name: "basetable",
    data() {
        return {
    ...
    methods: {
        ...
        handleSearch() {
            ...
            getGoodBySear(this.query.name).then(res=>{
                this.tableData=res.data;
            })
        ...
        handleCheck(index,row) {
            ...
            this.$confirm("确定要通过吗？", "提示", {
                ...
                .then(() => {
                    this.checkData=row;
                    this.checkData.goodssellstatus=1;
                    console.log(this.checkData)
                    modifyGood(this.checkData).then(res=>{
                        console.log(res)
